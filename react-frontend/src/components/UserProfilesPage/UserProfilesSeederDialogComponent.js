...
import seederData from "./userProfiles.seeder.json";
...
const UserProfilesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("userProfiles").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(UserProfilesSeederDialogComponent);
